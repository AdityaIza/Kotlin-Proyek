...
fun manipulateString(str: String, int: Int): String {
    val containsNumber = str.any { it.isDigit() }

    return if (containsNumber) {
        val numberInstring = Regex("^\\D*([0-9]+)").find(str)?.groupValues?.get(1)?.toIntOrNull() ?: 0

        str.replace(Regex("\\d+"), (numberInstring * int).toString())
    } else {
        "$str$int"
    }
}
